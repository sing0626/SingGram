...
//        FileLog.e("FileLoadOperation " + getFileName() + " setStream(" + stream + ")");
    ...
    private void updateParams() {
        boolean boost = SingGramConfig.isDownloadBoostEnabled();
        if ((preloadPrefixSize > 0 || MessagesController.getInstance(currentAccount).getfileExperimentalParams || boost) && !forceSmallChunk) {
            ...
            maxDownloadRequests = boost ? SingGramConfig.getBoostedDownloadRequestCount(8) : 8;
            maxDownloadRequestsBig = boost ? SingGramConfig.getBoostedDownloadRequestCount(8) : 8;
        } else {
