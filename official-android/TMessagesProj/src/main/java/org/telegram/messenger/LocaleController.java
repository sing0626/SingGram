...
public class LocaleController {
    ...
    public LocaleController() {
        ...
        addRules(new String[]{"az", "bm", "fa", "ig", "hu", "ja", "kde", "kea", "ko", "my", "ses", "sg", "to",
                "tr", "vi", "wo", "yo", "zh", "yue", "bo", "dz", "id", "jv", "jw", "ka", "km", "kn", "ms", "th", "in"}, new PluralRules_None());
        ...
        localeInfo.nameEnglish = "Traditional Chinese (Hong Kong)";
        localeInfo.shortName = "zh_hk";
        localeInfo.pluralLangCode = null;
        localeInfo.pathToFile = null;
        localeInfo.builtIn = true;
        languages.add(localeInfo);
        languagesDict.put(localeInfo.shortName, localeInfo);
        ...
        localeInfo = new LocaleInfo();
        localeInfo.name = "粵語";
        localeInfo.nameEnglish = "Cantonese";
        localeInfo.shortName = "yue_hant_hk";
        localeInfo.pluralLangCode = null;
        localeInfo.pathToFile = null;
        localeInfo.builtIn = true;
        languages.add(localeInfo);
    ...
    public int applyLanguage(final LocaleInfo localeInfo, boolean override, boolean init, boolean fromFile, boolean force, final int currentAccount, Runnable onDone) {
        ...
        try {
            ...
            if (args.length >= 3) {
                newLocale = Locale.forLanguageTag(shortName.replace('_', '-'));
    ...
    private String getStringInternal(String key, String fallback, int fallbackRes, int res) {
        String value = BuildVars.USE_CLOUD_STRINGS && !isSingGramBrandString(key, res) ? localeValues.get(key) : null;
        if (value == null) {
            if (BuildVars.USE_CLOUD_STRINGS && fallback != null && !isSingGramBrandString(fallback, fallbackRes)) {
    ...
    private static boolean isSingGramBrandString(String key, int res) {
        return res == R.string.AppName ||
                res == R.string.AppNameBeta ||
                res == R.string.TelegramPremium ||
                res == R.string.TelegramStars ||
                res == R.string.TelegramBusiness ||
                res == R.string.TelegramFAQ ||
                res == R.string.TelegramFeatures ||
                res == R.string.TelegramVersion ||
                "AppName".equals(key) ||
                "AppNameBeta".equals(key) ||
                "TelegramPremium".equals(key) ||
                "TelegramStars".equals(key) ||
                "TelegramBusiness".equals(key) ||
                "TelegramFAQ".equals(key) ||
                "TelegramFeatures".equals(key) ||
    ...
    public static String formatString(String key, String fallback, int res, int fallbackRes, Object... args) {
        try {
            String value = BuildVars.USE_CLOUD_STRINGS && !isSingGramBrandString(key, res) ? getInstance().localeValues.get(key) : null;
            if (value == null) {
                if (BuildVars.USE_CLOUD_STRINGS && fallback != null && !isSingGramBrandString(fallback, fallbackRes)) {
