    ...
    public BlurredBackgroundDrawable() {
        ...
        if (SingGramConfig.isLiquidGlassEnabled()) {
            if (SingGramConfig.isLiquidGlassCustomEnabled()) {
                boundProps.liquidThickness = (int) dpf2(SingGramConfig.getLiquidGlassThicknessDp());
                boundProps.liquidIntensity = SingGramConfig.getLiquidGlassIntensityPermille() / 1000.0f;
                boundProps.liquidIndex = SingGramConfig.getLiquidGlassIndexPermille() / 1000.0f;
            } else if (SingGramConfig.getLiquidGlassLevel() <= 0) {
                boundProps.liquidThickness = (int) dpf2(8);
                boundProps.liquidIntensity = 0.55f;
                boundProps.liquidIndex = 1.25f;
            } else if (SingGramConfig.getLiquidGlassLevel() >= 2) {
                boundProps.liquidThickness = (int) dpf2(14);
                boundProps.liquidIntensity = 0.95f;
                boundProps.liquidIndex = 1.85f;
            } else {
                boundProps.liquidThickness = (int) dpf2(11);
                boundProps.liquidIntensity = 0.75f;
                boundProps.liquidIndex = 1.5f;
            }
        }
        if (!SingGramConfig.isLiquidGlassCustomEnabled() && SingGramConfig.isLiquidGlassStrongEnabled()) {
            boundProps.liquidThickness = (int) dpf2(14);
            boundProps.liquidIntensity = 0.95f;
            boundProps.liquidIndex = 1.85f;
        }
